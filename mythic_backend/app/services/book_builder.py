...
async def generate_text_pages(run_id: str, style: str,
    ...
    try:
        ...
        system_prompt = f"""
Ты — профессиональный писатель романтических историй и создатель персональных альбомов. 
Твоя задача — создать невероятно красивый и эмоциональный рассказ для флипбука о человеке по имени {full_name}.

Ты получишь данные профиля Instagram и список изображений.
Для КАЖДОГО изображения напиши красивый, романтичный и ЛИЧНЫЙ текст (4-5 абзацев, 400-500 слов).

ВАЖНЫЕ ТРЕБОВАНИЯ:
1. Обращайся к {full_name} на "ты", используй имя
2. Пиши как влюбленный поклонник/тайный восхищенный человек
3. Каждый текст должен быть связан с предыдущим
4. Используй реальную информацию из профиля 
5. Пиши тепло, нежно, с восхищением
6. Описывай не только внешность, но и характер, энергетику, особенности
7. Убирай все хэштеги из текста

СТИЛЬ: романтичный, восхищенный, личный, поэтичный, искренний

ВЕРНИ ТОЛЬКО чистый JSON (без ```json или других обрамлений):
{{
  "pages": [
    "Текст для первого изображения...",
    "Текст для второго изображения...",
    ...
  ]
}}

Количество элементов в "pages" = {len(image_names)}. Ничего лишнего не добавляй.
        """.strip()
        ...
        resp = await async_client.chat.completions.create(
            ...
            temperature=0.8,  # Увеличиваем для более креативного контента
            max_tokens=6000,  # Увеличиваем для более длинных текстов (4-5 абзацев на каждое фото)
        )
        ...
        try:
            ...
            # Сначала пробуем найти JSON в тройных кавычках ```json
            json_match = re.search(r'```json\s*(\{[\s\S]*?\})\s*```', raw, re.MULTILINE | re.DOTALL)
            if json_match:
                try:
                    payload = json.loads(json_match.group(1))
                    print(f"✅ JSON успешно извлечен из тройных кавычек")
                except json.JSONDecodeError as e:
                    print(f"❌ JSON в тройных кавычках невалиден: {e}")
                    # Пробуем обычный regex как fallback
                    match = re.search(r'\{[\s\S]*\}', raw)
                    if match:
                        try:
                            payload = json.loads(match.group(0))
                            print(f"✅ JSON извлечен обычным regex")
                        except json.JSONDecodeError:
        ...
        if not isinstance(pages, list) or not pages or not all(isinstance(p, str) for p in pages):
            raise ValueError(f"JSON-ответ от LLM не содержит корректного списка 'pages': {payload}")

        # Проверяем, что количество страниц совпадает
        if len(pages) != len(image_names):
            print(f"⚠️ ВНИМАНИЕ: LLM вернул {len(pages)} страниц, а ожидалось {len(image_names)}. Дополняю недостающие...")
            
            # Если страниц меньше - дублируем последнюю
            while len(pages) < len(image_names):
                if pages:
                    pages.append(pages[-1])  # Дублируем последнюю страницу
                else:
                    pages.append("Эта страница наполнена особой красотой.")
            
            # Если страниц больше - обрезаем
            pages = pages[:len(image_names)]

        print(f"✅ LLM сгенерировал {len(pages)} текстов для {len(image_names)} изображений")
        return pages
        
    except Exception as e:
        print(f"⚠️ Ошибка в generate_text_pages: {e}")
        print("📝 Используется fallback текстов для всех страниц")
        
        # Fallback: создаем красивые тексты для каждой страницы
        full_name = "этого человека"
        try:
            posts_json_path = Path("data") / run_id / "posts.json"
        ...
        # Создаем набор красивых fallback текстов
        fallback_templates = [
            f"""Эта фотография рассказывает историю {full_name} — человека, который умеет находить красоту в каждом дне.

            Взгляни на этот кадр внимательно. Здесь запечатлена не просто сцена из жизни, а момент искренности и естественности. {full_name} живёт ярко, смело, с открытым сердцем, и это видно в каждой детали.

            В этом снимке отражается энергия, которая делает обычные моменты особенными. Когда смотришь на {full_name}, понимаешь, что настоящая красота — это не позы и фильтры, а внутренний свет, который невозможно скрыть.

            Это начало удивительной истории о человеке, который дарит миру частичку своей души в каждом кадре.""",
            
            f"""Каждый новый день приносит {full_name} новые возможности удивлять мир своей искренностью и открытостью.

            На этой фотографии время словно замерло, сохранив тот самый момент, когда счастье становится видимым каждому. Улыбка, взгляд, движение — всё говорит о том, что жизнь прекрасна, когда ты умеешь её ценить.

            {full_name} обладает редким даром — превращать простые мгновения в драгоценные воспоминания. Эта способность видеть красоту там, где другие видят обыденность, делает каждую встречу с {full_name} особенной.

            Этот кадр — живое доказательство того, что счастье не в больших событиях, а в умении радоваться каждому дню.""",
            
            f"""Время течёт быстро, но некоторые моменты остаются с нами навсегда. Эта фотография — один из таких моментов.

            Здесь запечатлена не просто картинка, а эмоция, которая передаётся через каждый пиксель. {full_name} умеет быть собой — искренне, без масок и притворства, и именно поэтому эти кадры так трогают сердце.

            Каждый день {full_name} доказывает, что жизнь стоит того, чтобы её проживать полно и ярко. Этот снимок — маленькое окно в мир, где каждый момент ценится и запоминается.

            Такие люди, как {full_name}, напоминают всем вокруг, что счастье — это не конечная цель, а образ жизни.""",
            
            f"""В жизни {full_name} каждая фотография — это страница в большой книге воспоминаний, которая пишется день за днём.

            Этот кадр особенный, потому что в нём есть та самая магия обычного момента, превращённого в произведение искусства. {full_name} не просто живёт — он вдохновляет других своим примером.

            Когда смотришь на эту фотографию, чувствуешь ту энергию жизни, которая исходит от {full_name}. Это энергия творчества, радости и бесконечной любви к миру вокруг.

            Каждый такой момент — это подарок, который {full_name} дарит не только себе, но и всем, кто имеет счастье быть частью этой истории."""
        ]
        
        # Создаем тексты для каждого изображения
        result_pages = []
        for i in range(len(image_names)):
            template = fallback_templates[i % len(fallback_templates)]
            result_pages.append(template)
            
        print(f"✅ Создано {len(result_pages)} fallback текстов")
        return result_pages
