...
ЗАДАЧА: Определи пол и выбери правильные русские местоимения.
...
Анализируй имя, учитывай результат анализа фото. Если имя женское (Анна, Мария, Арина, Emma, Sofia, Aida и т.д.) - выбирай female. Если мужское (Александр, Дмитрий, John, Azamat и т.д.) - выбирай male."""

        result = generate_text(prompt, max_tokens=150, temperature=0.1)
        ...
        try:
            ...
            if start != -1 and end != -1:
                ...
                if all(field in gender_data for field in required_fields):
                    print(f"✅ ИИ определил пол: {gender_data['gender']} ({gender_data['she_he']})")
                    return gender_data
                    
        except Exception as parse_error:
            print(f"❌ Ошибка парсинга JSON от ИИ: {parse_error}")
        
        # Fallback - простая логика если ИИ не смог
        print("⚡ ИИ не смог определить, использую fallback")
        if "female" in result.lower() or detected_gender == "female":
            return {
                "gender": "female",
