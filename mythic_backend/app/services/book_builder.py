...
Сначала напиши короткий, тёплый пролог для всей книги (2-3 предложения).
Затем, для КАЖДОГО изображения из списка `images`, напиши связный, подробно описанный рассказ — примерно 120–150 слов (3–4 коротких абзаца).
...
{{
  ...
  "pages": [
    "Текст для первой фотографии...",
    "Текст для второй фотографии...",
    "Текст для третьей фотографии...",
    ...
  ]
}}

Количество элементов в массиве "pages" должно СТРОГО соответствовать количеству элементов в массиве "images" ({len(image_names)}).
Не добавляй ничего лишнего. Только JSON.
    ...
    if len(pages) != len(image_names):
        print(f"⚠️ ВНИМАНИЕ: LLM вернул {len(pages)} страниц, а ожидалось {len(image_names)}. Тексты могут не соответствовать картинкам.")
