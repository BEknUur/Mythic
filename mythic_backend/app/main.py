...
def status(run_id: str):
    ...
    log.info(f"  posts_json exists: {posts_json.exists()}")
    log.info(f"  images_dir exists: {images_dir.exists()}")
    log.info(f"  images count: {len(list(images_dir.glob('*'))) if images_dir.exists() else 0}")
    log.info(f"  html_file exists: {html_file.exists()}")
    log.info(f"  pdf_file exists: {pdf_file.exists()}")
    ...
    status_info = {
        ...
        "stages": {
            ...
            "book_generated": html_file.exists()  # Изменено: проверяем HTML вместо PDF
        },
        "files": {}
    }
